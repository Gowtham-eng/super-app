...
const AppLauncher = () => {
  ...
  const launchApp = (app) => {
    ...
    if (app.type === 'saml' && token) {
      ...
      // OIDC apps: navigate to home_url directly (session managed by OIDC)
  ...
  return (
    <div className="animate-fadeIn" data-testid="app-launcher">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mb-5">{filtered.length} app{filtered.length !== 1 ? 's' : ''} available</p>
