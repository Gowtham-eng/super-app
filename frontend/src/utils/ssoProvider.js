/** Pick which Azure/Google config to start. Never guess "Refex" when several tenants exist. */
...
export function pickSsoProvider(providers, email = '') {
  ...
  if (!domain) return { provider: null, reason: 'need_email' };
  ...
  return { provider: null, reason: 'no_match', domain };
}
